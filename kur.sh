#!/usr/bin/env bash
# kur.sh — PTT UETS E-İmza'yı Apple Silicon Mac'te tek komutla derleyip kuran yardımcı betik.
#
...
REPO_URL="https://github.com/saidsurucu/uets-eimza-mac-arm64.git"
CLONE_DIR="$HOME/uets-eimza-mac-arm64"
...
if pgrep -f "$APP_NAME/Contents/MacOS" >/dev/null 2>&1; then
	...
	osascript -e 'tell application "PTT UETS E-İmza" to quit' >/dev/null 2>&1 || true
	sleep 2
fi
if [ -e "$DEST_APP" ]; then
...
# ----- Bitti -----
printf '\n'
ok "${BOLD}BİTTİ.${RST} PTT UETS E-İmza artık Launchpad ve Applications'ta."
say "Açmak için: ${BOLD}open \"$DEST_APP\"${RST}"
printf '\n'
warn "Bu uygulama ${BOLD}yalnızca UETS (e-Tebligat)${RST} içindir."
say "  PTT KEP Webmail için ayrı uygulama gerekir: github.com/saidsurucu/pttkep-eimza-mac-arm64"
printf '\n'
